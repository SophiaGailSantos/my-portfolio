...
const GlowCursor = ({
  ...
  useEffect(() => {
    ...
    const render = now => {
      ...
      const fadeStep =
        (16.667 * delta) /
        Math.max(config.fadeDuration, 16);
      ...
      program.uniforms.uSecondaryColor.value =
        hexToRgb(config.secondaryColor);

      program.uniforms.uTrailWidth.value =
      ...
      program.uniforms.uGlowIntensity.value =
        Math.max(config.glowIntensity, 0);

      program.uniforms.uGlowSpread.value =
        Math.max(config.glowSpread, 0);

      program.uniforms.uHotspot.value =
        clamp(config.hotspot, 0, 1);

      program.uniforms.uBrightness.value =
        Math.max(config.brightness, 0);

      program.uniforms.uOpacity.value =
        clamp(config.opacity, 0, 1);

      program.uniforms.uPulseSpeed.value =
        config.pulseSpeed;

      program.uniforms.uNoiseStrength.value =
  ...
  return (
    ...
    >
      ...
      {children && (
        <div className="glow-cursor__content">
          {children}
        </div>
      )}
    </div>
